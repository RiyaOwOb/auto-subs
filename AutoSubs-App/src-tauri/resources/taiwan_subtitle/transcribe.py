...
Windows/NVIDIA uses PyTorch with TEA-ASR mini and sequential forced alignment.
Apple Silicon uses MLX with TEA-ASR and Qwen fallback checkpoints.
"""
...
def _run_command(command: Sequence[str]) -> subprocess.CompletedProcess[str]:
    try:
        ...
        raise RuntimeError(f"找不到系統工具：{command[0]}，請安裝 FFmpeg 並確認 ffmpeg / ffprobe 已加入 PATH。") from exc
...
def resolve_backend(backend: str) -> str:
    ...
    selected = ("mlx" if apple else "cuda") if backend == "auto" else backend
    if selected == "mlx" and not apple:
        raise RuntimeError("MLX 需要 Apple Silicon Mac；Windows 請使用 --backend cuda。")
    return selected


def load_asr_model(model_id: str, backend: str = "mlx", *, aligner: bool = False) -> tuple[Any, ModelInfo]:
    configure_huggingface_cache()
    if backend == "cuda":
        from cuda_backend import load_model
        started = time.perf_counter()
        model = load_model(model_id, aligner=aligner)
        return model, ModelInfo(name=model_id, backend="cuda", load_seconds=time.perf_counter() - started,
                                compatibility=["FP16, eager attention, batch size 1"])
...
def align_all_chunks(
    ...
    aligner = None
    try:
        aligner, _ = load_asr_model(aligner_model_id, backend, aligner=True)
...
def run_transcription(
    ...
    if backend == "cuda":
        from cuda_backend import check_cuda, DEFAULT_ASR_MODEL, DEFAULT_ALIGNER_MODEL as CUDA_ALIGNER
        check_cuda()
        asr_model = asr_model or DEFAULT_ASR_MODEL
        aligner_model = aligner_model or CUDA_ALIGNER
    else:
        aligner_model = aligner_model or DEFAULT_ALIGNER_MODEL
    if chunk_seconds is None:
        chunk_seconds = 30.0 if backend == "cuda" else DEFAULT_CHUNK_SECONDS
    if not math.isfinite(chunk_seconds) or chunk_seconds <= 0:
        raise ValueError("chunk_seconds 必須為正有限數值")
    if not math.isfinite(overlap_seconds) or not 0 <= overlap_seconds < chunk_seconds:
        raise ValueError("overlap_seconds 必須介於 0 與 chunk_seconds 之間")
    ...
    try:
        ...
        for candidate in candidates:
            ...
            try:
                ...
                candidate_model, info = load_asr_model(candidate, backend)
        ...
        bundle = TranscriptBundle(
            ...
            aligner={"name": aligner_model, "backend": backend},
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="在 Windows NVIDIA / Apple Silicon Mac 上將影片/音訊轉成繁體中文字幕 SRT、TXT、JSON。"
    )
    ...
    parser.add_argument("--asr-model", default=None, help="指定單一 ASR model；CUDA 預設 TEA mini；MLX 依序探測 TEA/Qwen")
    parser.add_argument("--backend", choices=("auto", "cuda", "mlx"), default="auto")
    parser.add_argument("--aligner-model", default=None)
    parser.add_argument("--chunk-seconds", type=float, default=None, help="CUDA 預設 30 秒，MLX 預設 240 秒")
